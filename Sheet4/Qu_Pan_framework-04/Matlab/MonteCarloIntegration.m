...
for N_points=1:1:limit
    ...
    point_coords= -radius+2*radius*rand(2,N_points);
    x=point_coords(1,:);
    y=point_coords(2,:);
    ...
    % approximate area
    area_approx(N_points) =(2*radius)^2*inside_counter/N_points;
...
error = abs(area_approx-area_circle) ;
% plot the error
figure; 
plot(error);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for N_points=1:1:limit
    ...
    % randomly select N_points 2D points in the enclosing rectangle
    point_coords= -radius+2*radius*rand(3,N_points);
    x=point_coords(1,:);
    y=point_coords(2,:);
    z=point_coords(3,:);
    ...
    % approximate volume
    volume_approx(N_points) = (2*radius)^3*inside_counter/N_points;
...
% calculate the error for different numbers of points
error_volume = abs(volume_approx-volume);
